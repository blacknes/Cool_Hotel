	...
	$(function() {
		...
		var loginFun = function() {
			if ($('#loginDialog form').form('validate')) {
				...
				$.post('${pageContext.request.contextPath}/base/user!doNotNeedSessionAndSecurity_login.action', $('#loginDialog form').serialize(), function(result) {
					if (result.success) {
			...
			buttons : [ {
				...
				handler : function() {
					if ($('#passwordDialog form').form('validate')) {
						$.post('${pageContext.request.contextPath}/base/user!doNotNeedSecurity_updateCurrentPwd.action', {
							'data.pwd' : $('#pwd').val()
						}, function(result) {
							if (result.success) {
		...
		mainMenu = $('#mainMenu').tree({
			url : '${pageContext.request.contextPath}/base/resource!doNotNeedSecurity_getMainMenu.action',
			parentField : 'pid',
...
<body id="mainLayout" class="easyui-layout">
	<div data-options="region:'north',href:'<%=contextPath%>/admin/adminUp.jsp'" style="height: 70px; overflow: hidden;" class="logo"></div>
	...
	<div data-options="region:'center'" style="overflow: hidden;">
		<div id="mainTabs">
			<div title="首页" data-options="iconCls:'ext-icon-heart'">
				<iframe src="<%=contextPath%>/admin/adminIndex.jsp" allowTransparency="true" style="border: 0; width: 100%; height: 99%;" frameBorder="0"></iframe>
			</div>
		</div>
	</div>
	<div data-options="region:'south',href:'<%=contextPath%>/admin/adminBottom.jsp',border:false" style="height: 30px; overflow: hidden;"></div>
