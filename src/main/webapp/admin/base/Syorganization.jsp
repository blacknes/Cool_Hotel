...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sy.modalDialog({
			title : '添加机构信息',
			url : sy.contextPath + '/admin/base/SyorganizationForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '查看机构信息',
			url : sy.contextPath + '/admin/base/SyorganizationForm.jsp?id=' + id
		});
	};
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/organization!delete.action', {
					id : id
				}, function() {
	...
	var grantFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '机构授权',
			url : sy.contextPath + '/admin/base/SyorganizationGrant.jsp?id=' + id,
			buttons : [ {
				text : '授权',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	$(function() {
		grid = $('#grid').treegrid({
			title : '',
			url : sy.contextPath + '/base/organization!treeGrid.action',
			idField : 'id',
			treeField : 'name',
			parentField : 'pid',
			rownumbers : true,
