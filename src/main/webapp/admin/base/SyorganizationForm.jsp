...
<script type="text/javascript">
	var submitForm = function($dialog, $grid, $pjq) {
		if ($('form').form('validate')) {
			...
			if ($(':input[name="data.id"]').val().length > 0) {
				url = sy.contextPath + '/base/organization!update.action';
			} else {
				url = sy.contextPath + '/base/organization!save.action';
			}
	...
	$(function() {
		if ($(':input[name="data.id"]').val().length > 0) {
			...
			$.post(sy.contextPath + '/base/organization!getById.action', {
	...
	<form method="post" class="form">
		<fieldset>
			...
			<table class="table" style="width: 100%;">
				...
				<tr>
					<th>上级机构</th>
					<td><select id="syorganization_id" name="data.organization.id" class="easyui-combotree" data-options="editable:false,idField:'id',textField:'name',parentField:'pid',url:'<%=contextPath%>/base/organization!doNotNeedSecurity_comboTree.action'" style="width: 155px;"></select><img class="iconImg ext-icon-cross" onclick="$('#organization_id').combotree('clear');" title="清空" /></td>
					<th>机构图标</th>
