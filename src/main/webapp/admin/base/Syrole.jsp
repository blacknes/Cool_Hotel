...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sy.modalDialog({
			title : '添加角色信息',
			url : sy.contextPath + '/admin/base/SyroleForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '查看角色信息',
			url : sy.contextPath + '/admin/base/SyroleForm.jsp?id=' + id
		});
	};
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/role!delete.action', {
					id : id
				}, function() {
	...
	var grantFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '角色授权',
			url : sy.contextPath + '/admin/base/SyroleGrant.jsp?id=' + id,
			buttons : [ {
				text : '授权',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	$(function() {
		grid = $('#grid').datagrid({
			title : '',
			url : sy.contextPath + '/base/role!grid.action',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
